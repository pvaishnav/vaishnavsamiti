...
class ecard_gallery {
	...
	function show_ecard_gallery()
	{
		...
		$sqlQuery="SELECT * FROM album inner join category on album.album_type=category.cat_id and 
				...
				where category.cat_name='Ecard Gallery' and album.album_status='Y' 
		...
		$sqlQuery="select * from album_data inner join album  on album.album_id = album_data.album_id
				...
				where album_data.ad_deleted='N' and category.cat_name='Ecard Gallery'";
		
		$link="admin.php?view=ecard_gallery&action=show_ecard_gallery";
	...
	function add_ecard_gallery()
	{
		...
		redirect("admin.php?view=ecard_gallery&action=show_ecard_gallery");
		...
		if(isset($_REQUEST['submit']))
		{
			...
			if( !empty($_FILES['image_browse']['name'])	&& ( $_REQUEST['album_id'] !='0') )
			{
				...
				if (move_uploaded_file($_FILES['image_browse']['tmp_name'],$ImageName)) 
				{
					...
					{
						if(file_exists( "album_ecard_gallery/".$_REQUEST['ad_data'] ))
							unlink("album_ecard_gallery/".$_REQUEST['ad_data']);
